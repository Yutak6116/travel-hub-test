...
# いいねを追加
@plan_bp.route("/like_plan_item/<int:room_id>/<int:site_id>/like", methods=["POST"])
def like_plan_item(room_id, site_id):
    ...
    new_site = db.select(CandidateSite).where(CandidateSite.id == site_id)
    print(new_site)
    return redirect("/chat/" + str(room_id))


# いいねを取り消す
@plan_bp.route("/dislike_plan_item/<int:room_id>/<int:site_id>/dislike", methods=["POST"])
